...
def generate_anchors(base_anchor_size=16):
    ...
    ratios = np.array([0.597, 1.0, 1.675])
    # scales用于改变base_anchor_size的大小，三个不同的缩放比例
    scales = np.array([0.414, 0.52, 0.656])
    # anchor的数量是：scales*ratios所以共生成9个anchors
    num_anchors = len(ratios) * len(scales)

    # initialize output anchors
    anchors = np.zeros((num_anchors, 4))

    # 计算不同scales anchor的长和宽
    # np.tile():将array复制给定次数
